...
public class KhuCanHoBLL {
    ...
    public static TableModel show() {
        ...
        String[] columnNames = {"STT", "Mã Khu", "Khu", "Số Tầng", "Số căn/tầng","Loại Khu"};
    ...
    public static TableModel showPhanTich() {
        ArrayList<KhuCanHo> list = KhuCanHoDAL.showPhanTich();

        String[] columnNames = {"STT", "Mã Khu", "Khu", "Số Căn Hộ Trống", "Tỉ Lệ"};
        ...
        for (KhuCanHo khu : list) {
            ...
            data[i][1] = khu.getMaKhu();
            data[i][2] = khu.getTenKhu();
            data[i][3] = khu.getSoCanTrong();
            int multiply = khu.getSoCanTT() * khu.getSoTang();
            data[i][4] = ((float)khu.getSoCanTrong() / (float)multiply)*100;
            i++;
        }
    ...
    public static boolean insert(String tenKhu, int soTang, int soCanTT, String diaChi, float dienTich, long gia, int soPhong) {
        ...
        if (KhuCanHoDAL.show().isEmpty()) {
            maKhu = "A1";// chưa có khu nào lấy giá trị mặc định mã khu là EA
        } else {
